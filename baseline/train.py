...
def rand_bbox(size, lam):  # size : [Batch_size, Channel, Width, Height]
    W = size[2]
    H = size[3]
    cut_rat = np.sqrt(1.0 - lam)  # 패치 크기 비율
...
def train(data_dir, model_dir, args):
    ...
    for epoch in range(args.epochs):
        ...
        for idx, train_batch in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            if args.beta > 0 and np.random.random() > 0.3:  # cutmix가 실행될 경우
                ...
                outs = model(inputs)
                loss = criterion(outs, target_a) * lam + criterion(outs, target_b) * (
                    1.0 - lam
                )
        ...
        with torch.no_grad():
            ...
            wandb.log(
                {"Val_loss": val_loss, "Val_accuracy": val_acc, "results": figure}
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--epochs", type=int, default=100, help="number of epochs to train (default: 1)"
    )
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    parser.add_argument(
        ...
        default=os.environ.get(
            "SM_CHANNEL_TRAIN", "/opt/ml/Haewon/input/data/train/crop_images"
        ),
    )
    parser.add_argument(
        "--model_dir", type=str, default=os.environ.get("SM_MODEL_DIR", "./model")
    )
    parser.add_argument("--beta", default=0.5, help="training with cutmix if beta > 0")

    wandb.init(project="mask_contest", entity="sala0320")
    wandb.run.name = (
        "Efficient7 + CropImage + FocalLoss + CutMix 0.5 + AgeCut + CosineScheduler"
    )
